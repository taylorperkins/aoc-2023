...
def disentegration_scores(bricks: Dict[int, Brick]) -> int:
    def fn(current_bricks: Set[int], falling_bricks: Set[int], score: int) -> int:
        if not current_bricks:
            return score

        affected_bricks = set()
        for bid in current_bricks:
            brick = bricks[bid]
            for cid, child in brick.children.items():
                if cid not in falling_bricks | affected_bricks and len(set(child.parents) - falling_bricks) == 0:
                    affected_bricks.add(cid)
                    score += 1

        return fn(affected_bricks, falling_bricks | affected_bricks, score)

    total = 0
    for bid in bricks.keys():
        total += fn({bid}, {bid}, score=0)
    return total
...
def main(aoc: str):
    ...
    print(disentegration_scores(bricks))

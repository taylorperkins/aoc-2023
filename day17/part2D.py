...
class Path:
    ...
    def can_traverse(self, d: Direction) -> bool:
        if self.recent_direction is None:
            return True
        if d == REVERSE_DIRECTIONS.get(self.recent_direction):
            return False
        if d == self.recent_direction and self.recent_counter >= 10:
            return False
        if d != self.recent_direction and self.recent_counter < 4:
            return False
        return True
...
def main(aoc: str):
    ...
    # start real high
    min_heat_loss = shape[0] * shape[1] * 9
    ...
    while paths:
        ...
        for current_path in current_paths:
            ...
            if (
                (current_path.recent_counter, current_path.recent_direction,) not in closed_nodes[last_coord]
                and current_path.cost <= min_heat_loss
            ):
                ...
                for d in (up, down, left, right):
                    if current_path.can_traverse(d):
                        ...
                        if coord_is_valid(next_coord, shape) and next_coord not in current_path.visited:
                            ...
                            if next_coord == end and next_path.cost < min_heat_loss and 4 <= next_path.recent_counter <= 10:
                                min_heat_loss = next_path.cost
                                paths = {k: v for k, v in paths.items() if k < min_heat_loss}
                                continue

                            if (
                                (next_path.recent_counter, next_path.recent_direction) not in closed_nodes
                                and next_path.cost < min_heat_loss
                            ):
